...
import {
    Avatar, Box, Button, Card, CardActions, CardContent, Divider,
    Grid, InputAdornment, Pagination, TextField, Typography
} from "@mui/material";
    ...
    return (
        <Box sx={{ pb: 8 }}>
            <Box sx={{ mb: 4, textAlign: "center" }}>
                <Typography variant="h3" fontWeight={700} gutterBottom color="primary">{t("teams.title")}</Typography>
                <Typography variant="body1" color="text.secondary">{t("teams.subtitle")}</Typography>
            </Box>

            <Box sx={{ mb: 4, display: "flex", justifyContent: "flex-end" }}>
                <TextField
                    size="small" placeholder={t("teams.search_placeholder")} value={searchQuery}
            ...
            <Grid container spacing={3}>
                {paginatedData.length > 0 ? paginatedData.map((team) => (
                    <Grid size={{xs: 12, sm: 6, md: 4}} key={team.id}>
                        <Card
                            onClick={() => navigate(`/teams/${team.id}`)}
                            sx={{ height: "100%", display: "flex", flexDirection: "column", borderRadius: "16px", cursor: "pointer", border: "1px solid #e0e0e0", transition: "transform 0.2s, box-shadow 0.2s", "&:hover": { transform: "translateY(-4px)", boxShadow: "0 12px 30px rgba(0,0,0,0.1)", borderColor: "primary.main" } }}
                        >
                            <CardContent sx={{ flexGrow: 1, display: "flex", flexDirection: "column", gap: 2 }}>
                                <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                                    <Avatar sx={{ bgcolor: "primary.light", color: "primary.dark" }}><GroupsIcon /></Avatar>
                                    <Typography variant="h6" fontWeight={700} sx={{ wordBreak: "break-word" }}>{team.name}</Typography>
                                </Box>
                                <Divider />
                                <Box sx={{ display: "flex", alignItems: "center", color: "text.secondary" }}>
                                    <EmailIcon fontSize="small" sx={{ mr: 1 }} />
                                    <Typography variant="body2" sx={{ wordBreak: "break-all" }}>{team.email}</Typography>
                                </Box>
                            </CardContent>
                    ...
                    <Grid size={{xs: 12}}>
                        <Box sx={{ textAlign: "center", py: 10 }}>
                            ...
                            <Typography variant="h6" color="text.secondary">{t("teams.no_data")}</Typography>
                        </Box>
                    </Grid>
                )}
            </Grid>
