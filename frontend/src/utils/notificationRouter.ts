...
export const getNotificationLink = (notification: NotificationResponseDto | any): string => {
    if (notification.isGlobal) return '/notifications';

    try {
        const data = JSON.parse(notification.payload || '{}');
        const key = notification.key;

        if (key.includes('.round_event') || key.includes('.round_admin_message')) return `/rounds/${data.roundId}`;
        ...
        if (key.includes('.tournament')) return `/tournaments/${data.tournamentId}`;
